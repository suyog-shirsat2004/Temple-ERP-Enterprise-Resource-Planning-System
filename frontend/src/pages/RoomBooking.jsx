...
const RoomBooking = () => {
  ...
  const openModal = (room, index) => {
    ...
    setModalForm({ guest_name: '', phone: '', email: '', check_in: today, check_out: tomorrow, no_of_guests: 1 });
    setModalTotal(room.price_per_day || 0);
  ...
  const calculateModalTotal = (checkIn, checkOut, room) => {
    if (checkIn && checkOut && room) {
      ...
      if (nights > 0) {
        setModalTotal(nights * room.price_per_day);
      }
    }
  };
  ...
  const handleModalFormChange = (e) => {
    ...
    setModalForm((prev) => {
      ...
      if (name === 'check_in' || name === 'check_out') {
        calculateModalTotal(updated.check_in, updated.check_out, modalRoom);
      }
      return updated;
    });
  };
  ...
  const handleModalBooking = async (e) => {
    e.preventDefault();
    if (!modalRoom) return;
    if (modalForm.check_in && modalForm.check_out) {
      const checkInDate = new Date(modalForm.check_in);
      const checkOutDate = new Date(modalForm.check_out);
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '40px 20px 60px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 50 }}>
          <div style={{ textAlign: 'center', marginBottom: 40, color: '#fff' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.7)' }}>Click any room to start booking — {rooms.length} rooms available</p>
          </div>
          ...
          <div style={{
            ...
            {rooms.map((room, i) => {
              ...
              return (
                <div key={room._id || i} onClick={() => openModal(room, i)}
                  style={{
                ...
                >
                  <div style={{
                    ...
                    padding: '6px 12px', borderRadius: 15, fontSize: '0.75rem', fontWeight: 600,
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      {(() => {
                        const ci = new Date(modalForm.check_in);
                        const co = new Date(modalForm.check_out);
                        const validDates = !isNaN(ci.getTime()) && !isNaN(co.getTime());
                        const nights = validDates ? Math.ceil((co - ci) / (1000 * 60 * 60 * 24)) : 0;
                        return modalTotal > 0 && nights > 0
                          ? `₹${modalTotal.toLocaleString()} for ${nights} night(s)`
                          : 'Select Dates';
                      })()}
                    </div>
                  </div>
